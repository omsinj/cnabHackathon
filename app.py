...
from simulator import regenerate_file
from suggestions import suggest_fixes
from anomaly_detector import train_anomaly_model, detect_anomalies
...
def display_segment(segment, idx):
    ...
    try:
        amt = float(segment.get("fields", {}).get("payment_amount", 0))
        if amt > 50000:
            st.error("🚨 Suspicious: unusually high payment.")
    except ValueError:
        pass
...
def main():
    ...
    if uploaded_file:
        ...
        anomalies = detect_anomalies(parsed, model)
        ...
        if anomalies:
            ...
            for a in anomalies:
                st.markdown(f"- Batch {a['batch']} Segment {a['segment_index']}: {a['message']}")
        else:
        ...
        if st.button("🔧 Auto-Apply Fixes"):
            st.info("🔧 Auto-fix logic in development...")
        ...
        if st.button("📑 Export Validation Report (HTML)"):
            ...
            for fix in suggestions:
                html += f"<li style='color:orange'>{fix}</li>"
            for a in anomalies:
                html += f"<li style='color:darkred'>Anomaly: {a['message']}</li>"
            html += "</ul>"
